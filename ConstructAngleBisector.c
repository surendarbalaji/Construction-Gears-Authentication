...
AngleBisectorConstructor InitialiseAngleBisector(Vector2 pointB, Vector2 pointA, Vector2 pointC, float speed, float thickness, Color lengthColour, Color arcColour, Color equilateralLineColour, Color equilateralArcColour) {
    ...
    angleBisector.lengthColour = lengthColour;
    angleBisector.arcColour = arcColour;
    angleBisector.equilateralLineColour = equilateralLineColour;
    angleBisector.equilateralArcColour = equilateralArcColour;
    ...
    angleBisector.circleAD = InitialiseCircle(pointA, angleBisector.pointD, speed, thickness, arcColour, lengthColour);
    angleBisector.equilateralED = InitialiseEquilateral(angleBisector.pointE, angleBisector.pointD, angleBisector.pointA, speed, thickness, equilateralArcColour, equilateralLineColour);
...
void UpdateAngleBisector(AngleBisectorConstructor* angleBisector, float dt) {
    ...
    if (angleBisector->phase == ANGLEBISECTORCOMPLETE) return;

    switch (angleBisector->phase) {

        case CIRCLEAD:
            UpdateCircle(&angleBisector->circleAD, dt);
            if (angleBisector->circleAD.complete) angleBisector->phase = LINEDE;
            break;
...
void DrawAngleBisectorConstruction(const AngleBisectorConstructor* angleBisector) {
    ...
    DrawLineEx(angleBisector->pointA, angleBisector->pointB, angleBisector->thickness, angleBisector->lengthColour);
    DrawLineEx(angleBisector->pointA, angleBisector->pointC, angleBisector->thickness, angleBisector->lengthColour);
    ...
    switch (angleBisector->phase) {
        ...
        case LINEDE:
            ...
            DrawLineEx(angleBisector->pointD, Vector2Lerp(angleBisector->pointD, angleBisector->pointE, angleBisector->progress), angleBisector->thickness, angleBisector->lengthColour);
            break;
        ...
        case LINEAF:
            ...
            DrawLineEx(angleBisector->pointA, Vector2Lerp(angleBisector->pointA, angleBisector->equilateralED.pointC, angleBisector->progress), angleBisector->thickness, angleBisector->lengthColour);
            break;

        case ANGLEBISECTORCOMPLETE:
            DrawCircleConstruction(&angleBisector->circleAD);
            DrawEquilateralConstruction(&angleBisector->equilateralED);
            DrawLineEx(angleBisector->pointA, angleBisector->equilateralED.pointC, angleBisector->thickness, angleBisector->lengthColour);
            break;
    }
}